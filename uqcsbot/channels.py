...
class Channels(commands.Cog):
    ...
    def _channel_query(self, channel: str):
        ...
        channel_query = (
            ...
            .filter(Channel.name == channel, Channel.joinable == True)
            .one_or_none()
        )
        db_session.close()
    ...
    def _get_message_id(self):
        ...
        message_query = (
            ...
            .filter(Message.type == "react_message")
            .one_or_none()
        )
        db_session.close()
    ...
    channel_group = app_commands.Group(
        name="channel", description="Commands for joining and leaving channels"
    )
    ...
    async def join_command(self, interaction: discord.Interaction, channel: str):
        ...
        if channel_query == None:
            await interaction.response.send_message(
                f"Unable to join {channel}.", ephemeral=True
            )
            return
        ...
        if channel.permissions_for(member).is_superset(JOINED_PERMISSIONS):
            await interaction.response.send_message(
                f"You're already a member of {channel}.", ephemeral=True
            )
            return

        await channel.set_permissions(
            member, read_messages=True, reason="UQCSbot added."
        )
        join_message = await channel.send(
            f"{member.display_name} joined {channel.mention}"
        )
        await join_message.add_reaction("👋")
        await interaction.response.send_message(
            f"You've joined {channel.mention}.", ephemeral=True
        )

    @channel_group.command(name="leave")
    @app_commands.describe(
        channel="Channel you would like to leave. Defaults to current"
    )
    async def leave_command(
        ...
        if channel_query == None:
            await interaction.response.send_message(
                "Unable to leave that channel.", ephemeral=True
            )
            return
        ...
        if channel == None or channel.permissions_for(member).is_strict_subset(
            JOINED_PERMISSIONS
        ):
            await interaction.response.send_message(
                "Unable to leave that channel.", ephemeral=True
        ...
        await channel.set_permissions(
            member, read_messages=False, reason="UQCSbot removed."
        )
        await interaction.response.send_message(
            f"You've left {channel.mention}", ephemeral=True
        )
    ...
    @app_commands.describe(
        channelname="Channel to add as joinable",
        emoji="Emoji to associate with channel",
    )
    async def addjoinable_command(
        self, interaction: discord.Interaction, channelname: str, emoji: str
    ):
        """Sets a channel to be joinable with UQCSbot."""
        if not self._valid_emoji(emoji):
            await interaction.response.send_message(
                "Please select an emoji found within this server."
            )
            return
        ...
        taken = (
            db_session.query(Channel)
            .filter(Channel.id != channel.id, Channel.emoji == emoji)
            .first()
        )
        if taken:
            db_session.close()
            await interaction.response.send_message(
                "Please select an emoji that isn't already in use."
            )
            return

        existing = (
            db_session.query(Channel).filter(Channel.id == channel.id).one_or_none()
        )
        if existing:
            ...
            db_session.add(
                Channel(id=channel.id, name=channel.name, joinable=True, emoji=emoji)
            )
        db_session.commit()
        db_session.close()
        ...
        await interaction.response.send_message(
            f"{channel.mention} was added as a joinable channel."
        )
    ...
    async def removejoin_command(
        self, interaction: discord.Interaction, channelname: str
    ):
        ...
        try:
            ...
            await interaction.response.send_message(
                f"There was no record for {channel.mention}. The channel is not currently joinable."
            )
            return
        db_session.commit()
        db_session.close()
        ...
        await interaction.response.send_message(
            f"{channel.mention} was removed as a joinable channel."
        )
    ...
    async def setmessageid_command(
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if payload.message_id == self.message_id:
            ...
            channel_query = (
                db_session.query(Channel)
                .filter(Channel.joinable == True, Channel.emoji == emoji_name)
                .one_or_none()
            )
            db_session.close()
