...
class ManageCogs(commands.Cog):
    ...
    async def manage_cogs(self, interaction: discord.Interaction, action: Literal["load", "unload", "reload"], cog: str):
        """
        ...
        try:
            ...
            await interaction.response.send_message(f"Error occured {action}ing {cog}: {error}")
            return
