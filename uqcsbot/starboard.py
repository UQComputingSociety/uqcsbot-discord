...
class Starboard(commands.Cog):
    ...
    async def on_ready(self):
        ...
        if self.starboard_emoji is None or self.starboard_channel is None:
            logging.warning(
                f"Could not find required channel #{self.CHANNEL_NAME} or emoji {self.EMOJI_NAME}"
            )
    ...
    def _query_sb_message(self, recv: int) -> Optional[int]:
        ...
        id = (
            ...
            .filter(models.Starboard.recv == recv)
            .one_or_none()
        )
        db_session.close()
    ...
    def _create_sb_embed(self, recv: discord.Message) -> discord.Embed:
        embed = discord.Embed(
            color=recv.author.top_role.color, description=recv.content
        )
        embed.set_author(
            name=recv.author.display_name, icon_url=recv.author.display_avatar.url
        )
        ...
        if recv.reference is not None and not isinstance(
            recv.reference, discord.DeletedReferencedMessage
        ):
            ...
            replied.set_footer(
                text=recv.reference.resolved.created_at.strftime("%b %d, %H:%M:%S")
            )
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        ...
        if channel == self.starboard_channel or channel.category.name.startswith(
            "admin"
        ):
        ...
        if (
            new_reaction_count >= self.base_threshold
            and sb_message_id is None
            and recv_message.id not in self.base_blocked_messages
        ):
            ...
            new_reaction_count > self.base_threshold
            and sb_message_id is not None
            and recv_message.id not in self.big_blocked_messages
        ):
