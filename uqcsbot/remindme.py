...
REMINDER_LIMIT_REACHED_ERR = (
    f"You've reached the maximum number of active reminders ({USER_REMINDER_LIMIT})."
)
    ...
    def __str__(self):
        ...
        if self.week_frequency == 0:
            ...
            frequency = (
                ...
                if self.week_frequency == 1
                else f", repeating every {self.week_frequency} weeks"
            )
        ...
        return (
            f"Reminder with id {id} set for {day} at {time} starting {start_date} and "
...
def _error_embed(description: str, footer: str = "") -> discord.Embed:
    ...
    embed = discord.Embed(
        title=REMINDME_ERROR_TITLE, description=description, color=discord.Color.red()
    )
    if footer != "":
...
def _remove_reminder_embed(reminder: Reminder, footer: str = "") -> discord.Embed:
    ...
    embed = discord.Embed(
        title=REMINDME_REMOVED_TITLE, description=reminder.removed_str()
    )
    if footer != "":
        embed.set_footer(text=footer)
    return embed
...
def _list_reminders_embed(reminders: List[Reminder], footer: str = "") -> discord.Embed:
    ...
    if len(reminders) == 0:
        ...
        description = (
            f"You currently have {len(reminders)} active reminder(s).\n\n"
...
class RemindMe(commands.Cog):
    remindme_group = app_commands.Group(
        name="remindme", description="Reminder commands"
    )
    ...
    def _remove_reminder_from_db(self, reminder_id: int) -> Reminder:
        ...
        db_session = self.bot.create_db_session()
        reminder_query = (
            db_session.query(Reminders).filter(Reminders.id == reminder_id).one()
        )
    ...
    def _get_all_reminders(self) -> List[Reminder]:
        ...
        return [
            ...
            for reminder in sorted(
                reminders_query, key=lambda reminder: reminder.time_created
            )
        ]
    ...
    def _get_user_reminders(self, user_id: int) -> List[Reminder]:
        ...
        return list(
            filter(
                lambda reminder: reminder.user_id == user_id, self._get_all_reminders()
            )
        )
    ...
    def _schedule_reminder(self, reminder: Reminder):
        ...
        end_datetime = (
            dt.datetime.combine(reminder.end_date, time)
            if reminder.end_date != None
            else None
        )

        # if reminder datetime is in the past (bot downtime or something went wrong etc.), send a reminder now
        if (
            (reminder.week_frequency == None and start_datetime < dt.datetime.now())
            or reminder.week_frequency != None
    ...
    async def _process_reminder(self, reminder: Reminder):
        ...
        """
        if self.bot.uqcs_server != None:
            if (member := self.bot.uqcs_server.get_member(reminder.user_id)) != None:
                ctx = None
    ...
    async def add_reminder(
        ...
        if check_datetime < dt.datetime.now(tz=ZoneInfo("Australia/Brisbane")):
    ...
    async def add_recurring_reminder(
        ...
        try:
            check_time = dt.time.fromisoformat(time)
            check_start_date = (
                dt.date.fromisoformat(start_date) if start_date else dt.date.today()
            )
        ...
        if check_start_date < dt.date.today() or (
            check_end_date != None
            and dt.datetime.combine(check_end_date, check_time) < dt.datetime.now()
        ):
