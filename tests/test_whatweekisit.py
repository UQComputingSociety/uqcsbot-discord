...
from uqcsbot.whatweekisit import get_semester_times, get_semester_week, date_to_string, string_to_date
...
def test_get_semester_week_from_file(datafiles):
    ...
    _, week_name, weekday = get_semester_week(semesters, second_sem_week_one_day_two)
    assert(week_name == "Week 1")
    assert(weekday == "Tuesday")

    second_sem_break_day_four = string_to_date("29/09/2022")
    _, week_name, weekday = get_semester_week(semesters, second_sem_break_day_four)
